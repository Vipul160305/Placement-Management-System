...
const DrivesPage = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      addToast(err.message || 'Could not create drive', 'error');
    }
  };
              ...
              <div key={drive.id} className="card !p-0 overflow-hidden">
                <div className="p-5 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-bold text-lg">{initial}</div>
                    <div>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0 flex-wrap">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {drive.status === 'open' ? (
                        <button type="button" onClick={() => setStatus(drive.id, 'closed')} className="text-xs px-3 py-1.5 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50">
                          Close
                        </button>
                      ) : drive.status === 'closed' ? (
                        <button type="button" onClick={() => setStatus(drive.id, 'open')} className="text-xs px-3 py-1.5 rounded-lg border border-green-300 text-green-600 hover:bg-green-50">
                          Reopen
                        </button>
                      ) : (
                        <button type="button" onClick={() => setStatus(drive.id, 'open')} className="text-xs px-3 py-1.5 rounded-lg border border-primary/30 text-primary hover:bg-primary/5">
                          Open
                        </button>
                      )}
                      <button
                      ...
                      <button type="button" onClick={() => removeDrive(drive.id)} className="text-xs px-3 py-1.5 rounded-lg border border-red-200 text-red-600 hover:bg-red-50">
                        Delete
                      </button>
                    </div>
                  </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Create Placement Drive" size="xl">
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div>
            ...
            <input required value={form.title} onChange={f('title')} placeholder="e.g. Campus hiring 2026" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Job role</label>
              <input value={form.jobRole} onChange={f('jobRole')} placeholder="e.g. Software Engineer" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Package</label>
              <input value={form.package} onChange={f('package')} placeholder="e.g. 12 LPA" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
          </div>
          ...
          <div>
            ...
            <input value={form.allowedBranches} onChange={f('allowedBranches')} placeholder="CSE, ECE, IT" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
